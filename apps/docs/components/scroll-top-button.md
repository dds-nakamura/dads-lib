...
- **画面右下/左下の固定ボタン** は、モバイルでメインコンテンツの最終行を覆い隠す、フォーカスリングやリーダーモードと干渉する、といった問題を起こしやすい
- **スムーススクロール** は `prefers-reduced-motion` 設定で抑止されるが、抑止されたとしても瞬時のジャンプにユーザーが追従できないケースがある
- スクロール先頭への移動は **ブラウザ標準操作 (Home キー / アドレスバータップ / フッターから戻る導線)** で十分にカバーできる場面が大半

**代替の検討先**:

- 長文ページの読書補助 → `DadsTableOfContents` (目次)、`DadsPageNavigation` (前後ページ)
- セクション間ジャンプ → スキップリンク (`<a href="#main">`) + `DadsAnchor`
- ヘッダー追従 → `DadsHeaderContainer` の `sticky` モード

既存実装の互換性のため API は維持していますが、新規実装での採用は避けてください。
:::
