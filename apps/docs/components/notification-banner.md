...
5 つの semantic color (`info-1` / `info-2` / `success` / `warning` / `error`)。デフォルトは `info-1`。
公式 DADS の `data-type` 軸に一致 (`info-1` = プライマリブルー / `info-2` = ニュートラルグレー)。
`error` / `warning` は `role="alert"`、それ以外は `role="status"` が自動付与される。

<div class="demo">
  <span class="demo-label">Info 1 (default)</span>
  <DadsNotificationBanner color="info-1" message="新しい機能が公開されました。" :closable="false" />
  <span class="demo-label" style="margin-top:1rem">Info 2</span>
  <DadsNotificationBanner color="info-2" message="メンテナンスのお知らせです。" :closable="false" />
...
<div class="demo">
  ...
  <DadsNotificationBanner color="info-1" message="標準スタイル" :closable="false" />
  <span class="demo-label" style="margin-top:1rem">style="color-chip"</span>
  <DadsNotificationBanner color="info-1" style="color-chip" message="カラーチップスタイル" :closable="false" />
...
<DadsNotificationBanner color="error" title="送信に失敗しました">
  ネットワーク接続を確認してください。
  <template #action>
    <DadsButton size="sm" variant="outline">再試行</DadsButton>
  </template>
</DadsNotificationBanner>
```
...
| `color`      | `'success' \| 'error' \| 'warning' \| 'info-1' \| 'info-2'` | `'info-1'`   | セマンティックカラー (公式 `data-type` 軸)。アクセント色・既定アイコンを決定 |
...
- `color="success"` / `"info-1"` / `"info-2"` は `role="status"` + `aria-live="polite"`
