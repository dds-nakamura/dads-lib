...
import DadsHeader from '../DadsHeader.vue'
import type { DadsHeaderProps } from '../DadsHeader.types'

enableAutoUnmount(afterEach)

const createWrapper = (props: DadsHeaderProps = {}, slots: Record<string, string> = {}) =>
  mount(DadsHeader, {
    props,
    slots,
...
describe('DadsHeader', () => {
  describe('rendering', () => {
    it('renders a <header> element as the root', () => {
      ...
      expect(wrapper.classes()).toContain('dads-header')
    })

    it('renders the inner layout wrapper', () => {
      const wrapper = createWrapper()
      expect(wrapper.find('.dads-header__inner').exists()).toBe(true)
    })
  })
  ...
  describe('sticky prop', () => {
    it('applies the sticky modifier class by default', () => {
      const wrapper = createWrapper()
      expect(wrapper.classes()).toContain('dads-header--sticky')
    })

    it('omits the sticky modifier class when sticky=false', () => {
      const wrapper = createWrapper({ sticky: false })
      expect(wrapper.classes()).not.toContain('dads-header--sticky')
    })
  })
  ...
  describe('showMenuToggle prop', () => {
    it('renders the hamburger button by default', () => {
      const wrapper = createWrapper()
      expect(wrapper.find('.dads-header__menu-toggle').exists()).toBe(true)
    })

    it('hides the hamburger button when showMenuToggle=false', () => {
      const wrapper = createWrapper({ showMenuToggle: false })
      expect(wrapper.find('.dads-header__menu-toggle').exists()).toBe(false)
    })
  })
  ...
  describe('menuToggleLabel prop', () => {
    it('uses the default Japanese aria-label', () => {
      const wrapper = createWrapper()
      const button = wrapper.find('.dads-header__menu-toggle')
  ...
  describe('click:menu emit', () => {
    it('emits click:menu when the hamburger is clicked', async () => {
      const wrapper = createWrapper()
      await wrapper.find('.dads-header__menu-toggle').trigger('click')
  ...
  describe('hamburger icon accessibility', () => {
    it('marks the visual icon as aria-hidden', () => {
      const wrapper = createWrapper()
      const icon = wrapper.find('.dads-header__menu-toggle .mdi-menu')
  ...
  describe('nav slot', () => {
    it('does not render the nav wrapper when slot is empty', () => {
      const wrapper = createWrapper()
      expect(wrapper.find('.dads-header__nav').exists()).toBe(false)
    })
    ...
    it('renders the nav wrapper as a <nav> element with aria-label and slot content', () => {
      ...
      const nav = wrapper.find('.dads-header__nav')
  ...
  describe('actions slot', () => {
    it('does not render the actions wrapper when slot is empty', () => {
      const wrapper = createWrapper()
      expect(wrapper.find('.dads-header__actions').exists()).toBe(false)
    })
    ...
    it('renders the actions wrapper and content when slot is provided', () => {
      ...
      const actions = wrapper.find('.dads-header__actions')
  ...
  describe('all slots together', () => {
    it('renders logo, nav, and actions wrappers when all slots are populated', () => {
      ...
      expect(wrapper.find('.dads-header__logo').exists()).toBe(true)
      expect(wrapper.find('.dads-header__nav').exists()).toBe(true)
      expect(wrapper.find('.dads-header__actions').exists()).toBe(true)
    })
  })
})
