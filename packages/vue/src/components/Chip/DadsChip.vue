...
import type { DadsChipEmits, DadsChipProps } from './DadsChip.types'

const props = withDefaults(defineProps<DadsChipProps>(), {
  size: 'md',
  color: 'primary',
  closable: false,
  clickable: false,
...
const emit = defineEmits<DadsChipEmits>()

const rootClasses = computed(() => [
  'dads-chip',
  `dads-chip--${props.size}`,
  `dads-chip--${props.color}`,
  {
    'dads-chip--clickable': props.clickable,
  },
])

// `aria-disabled` is only meaningful when the chip itself is not a real
...
<template>
  <component
    ...
    @keydown="onKeydown"
  >
    <span v-if="$slots.prepend" class="dads-chip__prepend" aria-hidden="true">
      <slot name="prepend" />
    </span>
    <span class="dads-chip__label">
      <slot />
    </span>
    <span v-if="$slots.append && !closable" class="dads-chip__append" aria-hidden="true">
      <slot name="append" />
    </span>
    <button
      v-if="closable"
...
// background so chips read as quiet labels rather than buttons.
$dads-chip-colors: (
  primary: (
    --color-brand-primary,
    --color-info-bg,
...
.dads-chip {
  ...
  // -------------------- disabled ------------------------------------------
  // Mirrors the DadsButton rule so disabled chips stay out of :hover/:active
  // styling without a chain of :not() selectors.
  &:disabled,
  &[aria-disabled='true'] {
    cursor: not-allowed;
    opacity: 0.5;
  ...
  @each $name, $tokens in $dads-chip-colors {
