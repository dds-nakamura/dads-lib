...
const createWrapper = (
  props: DadsChipTagProps = {},
  slots: Record<string, string> = { default: 'タグ' },
) => mount(DadsChipTag, { props, slots, attachTo: document.body })

describe('DadsChipTag', () => {
  describe('rendering', () => {
    it('renders a span by default', () => {
      const wrapper = createWrapper()
      expect(wrapper.element.tagName).toBe('SPAN')
      expect(wrapper.classes()).toContain('dads-chip-tag')
    })
    ...
    it('renders the default slot inside the label element', () => {
      ...
      const label = wrapper.find('.dads-chip-tag__label')
    ...
    it('applies default size and color classes', () => {
      const wrapper = createWrapper()
      expect(wrapper.classes()).toContain('dads-chip-tag--md')
      expect(wrapper.classes()).toContain('dads-chip-tag--primary')
    })
  })

  describe('size prop', () => {
    it.each(['lg', 'md', 'sm'] as const)('applies dads-chip-tag--%s class', (size) => {
      const wrapper = createWrapper({ size })
      expect(wrapper.classes()).toContain(`dads-chip-tag--${size}`)
    })
  })

  describe('color prop', () => {
    it.each(['primary', 'success', 'error', 'warning', 'secondary'] as const)(
      'applies dads-chip-tag--%s class',
      (color) => {
        const wrapper = createWrapper({ color })
        expect(wrapper.classes()).toContain(`dads-chip-tag--${color}`)
      },
    )
  })
  ...
  describe('closable', () => {
    ...
    it('uses the default closeLabel "削除"', () => {
      const wrapper = createWrapper({ closable: true })
      expect(wrapper.find('.dads-chip-tag__close').attributes('aria-label')).toBe('削除')
    })

    it('honors a custom closeLabel', () => {
      const wrapper = createWrapper({ closable: true, closeLabel: 'remove tag' })
      expect(wrapper.find('.dads-chip-tag__close').attributes('aria-label')).toBe('remove tag')
    })

    it('emits close when the × button is clicked', async () => {
      const wrapper = createWrapper({ closable: true })
      await wrapper.find('.dads-chip-tag__close').trigger('click')
  ...
  describe('slots', () => {
    ...
    it('hides the append slot when closable=true', () => {
      ...
      expect(wrapper.find('.dads-chip-tag__append').exists()).toBe(false)
