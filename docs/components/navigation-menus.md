...
`@dads/vue` のナビゲーション / メニュー系コンポーネント 10 件の永続参照ドキュメント。DADS 公式仕様 (`dads-document-md/dads/components/<slug>/index.md`) を真実の源とし、HTML 実装サンプル (`design-system-example-components-html/src/components/<slug>/`) を実装参考としている。

## 対象コンポーネント

本ドキュメントが扱う 10 件のコンポーネントは、依存関係に基づき以下 2 グループに分類される。
...
| `scroll-top-button`     | `DadsScrollTopButton`     | トップへ戻るボタン       | なし        |
...
| `bottom-navigation` | `DadsBottomNavigation` | モバイル下部ナビ          | -                          | なし        |
| `mobile-menu`       | `DadsMobileMenu`       | モバイル用メニュー        | HamburgerMenuButton（任意）| なし        |

### スコープ外

- `header-container` / `drawer` — 既存 `DadsHeader` / `DadsDrawer` に統合済
...
- **landmark role**: グローバル系（`DadsGlobalMenu`, `DadsMegaMenu`, `DadsMobileMenu`, `DadsBottomNavigation`）は `<nav>` 要素 + `aria-label` でランドマーク化。`DadsPageNavigation` も同様（`aria-label="目次"` 等）。
...
- **モバイル切替**:
  ...
  - `DadsBottomNavigation` はモバイル専用（PC では非表示）。
  - 切り替えは CSS `@media` クエリで行い、JS による DOM 削除はしない（SSR / hydration 安定のため）。
- **ScrollTopButton**: ビューポート位置を `IntersectionObserver` で監視し、閾値を超えたら表示。SSR 安全のため `onMounted` 内で観測開始 / `onBeforeUnmount` でリソース解放する。
...
モバイル時のメニュー開閉ボタン。`DadsHeader` の `#showMenuToggle` slot で利用される pattern と互換。
...
- **既存 Header との互換**: `DadsHeader` の `showMenuToggle` slot 内で動作することを必須要件とする（依存破壊しない）。
...
ページトップへスクロールするフローティングボタン。

- **Props**:
  - `threshold?: number` — 表示開始 scrollY（既定: 200）
  - `target?: string | HTMLElement` — スクロール対象（既定: `window`）
  - `ariaLabel?: string` — 既定値「ページの先頭へ戻る」
  - `behavior?: 'smooth' | 'auto'` — `scrollTo` 挙動
- **Emits**:
  - `click` — ボタン押下時
- **実装注意**:
  - `onMounted` 内で `IntersectionObserver` または `window.addEventListener('scroll')` を登録、`onBeforeUnmount` で必ず `removeEventListener` / `disconnect()` を実行。
  - SSR 環境で `window` を参照しない（`<ClientOnly>` 不要、`onMounted` 内のみで window アクセス）。
- **a11y**: `<button type="button">` + 視覚的に固定配置でも `aria-label` を必須化。
...
### DadsBottomNavigation

モバイル下部固定ナビゲーション（最大 5 項目）。

- **Props**:
  - `items: BottomNavItem[]` — `{ label, href, icon: string, current?: boolean }`、上限 5 件
  - `ariaLabel?: string`
- **Slots**:
  - `#icon="{ item }"` — アイコンカスタマイズ（`@mdi/font` 等を想定）
- **Emits**:
  - `select(item)`
- **a11y**: `<nav aria-label="...">` + `<ul role="tablist">` 様の振る舞い。アイコンには `aria-hidden="true"`、ラベルテキストで意味を伝える。
- **アイコン依存**: `@mdi/font` のロード前提を `DadsButton` と同様にドキュメント注記。
- **レスポンシブ**: PC 表示では非表示。
