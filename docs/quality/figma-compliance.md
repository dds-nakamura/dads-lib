...
> **現行スナップショット**: DADS **v2.14.0** / **43 ページ** / `fileKey=k37j9LDbHk1LwpgdzTZJWY` / exportedAt 2026-06-13（playwright モード）。v2.13.0（42 ページ）から「スタイルガイド」ページが追加され、「グローバルメニュー」ページは「水平メニュー」に名称変更された。pin は `VENDORED.md` を正本とする。

### ファイル配置

```
dads-document-figma/
├── <ページ名>/<ページ名>.png      ← 全 43 件のビジュアル仕様
...
| `figma-playwright-export.mjs` | Playwright 経由で UI からエクスポート (推奨) | 43 ページ一括取得 / rate limit 回避が必要なとき |
...
- **43 ページ一括取得**: スナップショット全件 (43 PNG) を 1 コマンドで安定取得できる。
