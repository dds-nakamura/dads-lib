...
## 5. 要決定チェックリスト（このレビューで○×）

- [ ] **ポリシー**: 案X（公式準拠 major 統一）で良いか／案Y か
- [ ] **存続検討の機能**: Button `loading`、Select のリッチ選択、SearchBox `clear/suggestions`、DescriptionList `bordered` を **残す/削除** どちらにするか（個別）
- [ ] **Carousel autoPlay**: MD 違反のため削除で良いか
- [ ] **柱B との順序**: T5 着手前に DadsIcon を先行整備して良いか
- [ ] **PR 分割単位**: テーマ単位で良いか（T1 から）
- [ ] **T9 保留**: Figma 公開待ちで保留に同意するか
